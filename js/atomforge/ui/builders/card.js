 ...
 * preview(): returns { structure, caption, colorBy, shape, mode } for the current inputs
 *   (shape/mode: optional outline drawn with the atoms, see render/preview.js).
...
export function registerCard(id, { preview, build, busy = 'Building…' }) {
    ...
    const run = () => {
        ...
        try {
            const r = preview();
            previewRenderer().show(canvas, r.structure, { colorBy: r.colorBy || 'element', shape: r.shape || null, mode: r.mode || 'atoms' });
    ...
    // Builders are chosen with the tiles only: the card header is a plain title.
    const summary = el.querySelector('summary');
    summary.tabIndex = -1;
    summary.addEventListener('click', (e) => e.preventDefault());
    el.addEventListener('toggle', () => {
        if (!el.open) return;
        // One builder shown at a time.
