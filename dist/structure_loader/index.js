...
const atomsInfo = {
  H:  { radius: 0.31, color: 0xFFFFFF }, // White
  He: { radius: 0.28, color: 0xD9FFFF }, // Pale cyan
  Li: { radius: 1.28, color: 0xCC80FF }, // Violet
  Be: { radius: 0.96, color: 0xC2FF00 }, // Light green
  B:  { radius: 0.84, color: 0xFFB5B5 }, // Salmon
  C:  { radius: 0.76, color: 0x909090 }, // Dark gray
  N:  { radius: 0.71, color: 0x3050F8 }, // Blue
  O:  { radius: 0.66, color: 0xFF0D0D }, // Red
  F:  { radius: 0.57, color: 0x90E050 }, // Light green
  Ne: { radius: 0.58, color: 0xB3E3F5 }, // Light blue

  Na: { radius: 1.66, color: 0xAB5CF2 }, // Purple
  Mg: { radius: 1.41, color: 0x8AFF00 }, // Green
  Al: { radius: 1.21, color: 0xBFA6A6 }, // Light gray
  Si: { radius: 1.11, color: 0xF0C8A0 }, // Beige
  P:  { radius: 1.07, color: 0xFF8000 }, // Orange
  S:  { radius: 1.05, color: 0xFFFF30 }, // Yellow
  Cl: { radius: 1.02, color: 0x1FF01F }, // Green
  Ar: { radius: 1.06, color: 0x80D1E3 }, // Cyan
  K:  { radius: 2.03, color: 0x8F40D4 }, // Violet
  Ca: { radius: 1.76, color: 0x3DFF00 }, // Green

  Sc: { radius: 1.7,  color: 0xE6E6E6 }, // Gray (no CPK)
  Ti: { radius: 1.6,  color: 0xBFC2C7 }, // Silver
  V:  { radius: 1.53, color: 0xA6A6AB },
  Cr: { radius: 1.39, color: 0x8A99C7 },
  Mn: { radius: 1.50, color: 0x9C7AC7 },
  Fe: { radius: 1.42, color: 0xE06633 }, // Dark orange
  Co: { radius: 1.38, color: 0xF090A0 },
  Ni: { radius: 1.24, color: 0x50D050 },
  Cu: { radius: 1.32, color: 0xC88033 }, // Orange-brown
  Zn: { radius: 1.22, color: 0x7D80B0 },

  Ga: { radius: 1.22, color: 0xC28F8F },
  Ge: { radius: 1.20, color: 0x668F8F },
  As: { radius: 1.19, color: 0xBD80E3 },
  Se: { radius: 1.20, color: 0xFFA100 },
  Br: { radius: 1.20, color: 0xA62929 }, // Dark red
  Kr: { radius: 1.16, color: 0x5CB8D1 },

  Rb: { radius: 2.20, color: 0x702EB0 },
  Sr: { radius: 1.95, color: 0x00FF00 },
  Y:  { radius: 1.90, color: 0x94FFFF },
  Zr: { radius: 1.75, color: 0x94E0E0 },
  Nb: { radius: 1.64, color: 0x73C2C9 },
  Mo: { radius: 1.54, color: 0x54B5B5 },
  Tc: { radius: 1.47, color: 0x3B9E9E },
  Ru: { radius: 1.46, color: 0x248F8F },
  Rh: { radius: 1.42, color: 0x0A7D8C },
  Pd: { radius: 1.39, color: 0x006985 },

  Ag: { radius: 1.45, color: 0xC0C0C0 }, // Silver
  Cd: { radius: 1.44, color: 0xFFD98F },
  In: { radius: 1.42, color: 0xA67573 },
  Sn: { radius: 1.39, color: 0x668080 },
  Sb: { radius: 1.39, color: 0x9E63B5 },
  Te: { radius: 1.38, color: 0xD47A00 },
  I:  { radius: 1.39, color: 0x940094 }, // Purple
  Xe: { radius: 1.40, color: 0x429EB0 },

  Cs: { radius: 2.44, color: 0x57178F },
  Ba: { radius: 2.15, color: 0x00C900 },

  // Lanthanides & Actinides are not part of original CPK — kept gray or adjusted to distinct colors
  La: { radius: 2.07, color: 0x70D4FF },
  Ce: { radius: 2.04, color: 0xFFFFC7 },
  Pr: { radius: 2.03, color: 0xD9FFC7 },
  Nd: { radius: 2.01, color: 0xC7FFC7 },
  Pm: { radius: 1.99, color: 0xA3FFC7 },
  Sm: { radius: 1.98, color: 0x8FFFC7 },
  Eu: { radius: 1.98, color: 0x61FFC7 },
  Gd: { radius: 1.96, color: 0x45FFC7 },
  Tb: { radius: 1.94, color: 0x30FFC7 },
  Dy: { radius: 1.92, color: 0x1CFFC7 },
  Ho: { radius: 1.92, color: 0x00FF9C },
  Er: { radius: 1.89, color: 0x00E675 },
  Tm: { radius: 1.90, color: 0x00D452 },
  Yb: { radius: 1.87, color: 0x00BF38 },
  Lu: { radius: 1.87, color: 0x00AB24 },

  Hf: { radius: 1.75, color: 0x4DC2FF },
  Ta: { radius: 1.70, color: 0x4DA6FF },
  W:  { radius: 1.62, color: 0x2194D6 },
  Re: { radius: 1.51, color: 0x267DAB },
  Os: { radius: 1.44, color: 0x266696 },
  Ir: { radius: 1.41, color: 0x175487 },
  Pt: { radius: 1.36, color: 0xD0D0E0 }, // Gray blue
  Au: { radius: 1.36, color: 0xFFD123 }, // Gold
  Hg: { radius: 1.32, color: 0xB8B8D0 },

  Tl: { radius: 1.45, color: 0xA6544D },
  Pb: { radius: 1.46, color: 0x575961 },
  Bi: { radius: 1.48, color: 0x9E4FB5 },
  Po: { radius: 1.40, color: 0xAB5C00 },
  At: { radius: 1.50, color: 0x754F45 },
  Rn: { radius: 1.50, color: 0x428296 },

  Fr: { radius: 2.60, color: 0x420066 },
  Ra: { radius: 2.21, color: 0x007D00 },

  // Elements with no CPK value: use gray
  Ac: { radius: 2.15, color: 0xC8C8C8 },
  Th: { radius: 2.06, color: 0xC8C8C8 },
  Pa: { radius: 2.00, color: 0xC8C8C8 },
  U:  { radius: 1.96, color: 0x008FFF },
  Np: { radius: 1.90, color: 0x0080FF },
  Pu: { radius: 1.87, color: 0x006BFF },
  Am: { radius: 1.80, color: 0x545CF2 },
  Cm: { radius: 1.76, color: 0x785CE3 },
  Bk: { radius: 1.68, color: 0x8A4FE3 },
  Cf: { radius: 1.68, color: 0xA136D4 },
  Es: { radius: 1.65, color: 0xB31FD4 },
  Fm: { radius: 1.67, color: 0xB31FBA },
  Md: { radius: 1.73, color: 0xB30DA6 },
  No: { radius: 1.76, color: 0xBD0D87 },
  Lr: { radius: 1.61, color: 0xC70066 },
  Rf: { radius: 1.57, color: 0xCC0059 },
  Db: { radius: 1.49, color: 0xD1004F },
  Sg: { radius: 1.43, color: 0xD90045 },
  Bh: { radius: 1.41, color: 0xE00038 },
  Hs: { radius: 1.34, color: 0xE6002E },
  Mt: { radius: 1.29, color: 0xEB0026 },
  Ds: { radius: 1.28, color: 0xF0001C },
  Rg: { radius: 1.21, color: 0xF50012 },
  Cn: { radius: 1.22, color: 0xFA000A },
  Nh: { radius: NaN, color: 0xFF0000 },
  Fl: { radius: NaN, color: 0xFF6666 },
  Mc: { radius: NaN, color: 0xFF9999 },
  Lv: { radius: NaN, color: 0xFFB5B5 },
  Ts: { radius: NaN, color: 0xFFC0C0 },
  Og: { radius: NaN, color: 0xCCCCCC },
};
