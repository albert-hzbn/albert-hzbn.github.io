...
function convert_sqs_to_poscar() {
    const bestsqs = document.getElementById("bestsqs").value.trim();
    ...
    for (let index = 0; index < atomicPositions.length; index++) {
        ...
        const elem = atomPosStrArr.slice(3)[0];
    ...
    for (let elemIndex = 0; elemIndex < elemArr.length; elemIndex++) {
        ...
        for (let index = 0; index < atomicPositions.length; index++) {
            if (elemArr[elemIndex] === atomicPositions[index].split(" ")[3]) {
                const atomPosStrArr = atomicPositions[index].split(" ");
                const elemCoordStr = atomPosStrArr.slice(0, 3);
    ...
    let unitCellArr = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
    let latticeVectorArr = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
    ...
    document.getElementById("poscar").value = strPOSCAR;
}
